  ...
  <div class="section-card p-5 sm:p-6">
    ...
    <ol class="relative mt-6 space-y-0">
      <li v-for="(item, index) in items" :key="item.title" class="relative flex gap-4 pb-6 last:pb-0">
        ...
        <span class="relative z-10 grid size-9 shrink-0 place-items-center rounded-full border-2 transition" :class="item.done ? 'border-brand-500 bg-brand-500 text-white' : item.current ? 'border-brand-500 bg-[rgb(var(--surface))] text-brand-600 shadow-focus' : 'border-[rgb(var(--line))] bg-[rgb(var(--surface))] muted'">
