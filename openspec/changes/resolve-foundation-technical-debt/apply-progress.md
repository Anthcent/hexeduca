...
- [x] 3.8 Ran `storage:probe` → `PASS: upload, read-back, and delete all succeeded on the s3 disk.`; output grepped for `sail`/`password`/`AWS_SECRET`/`AKIA` — no matches (no credential leak).
