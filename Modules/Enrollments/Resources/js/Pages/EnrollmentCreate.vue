...
<template>
  <div class="p-6">
    ...
    <form class="flex flex-col gap-3 max-w-md" @submit.prevent="create">
      <select v-model.number="form.academic_offer_id" class="border rounded px-3 py-2">
      ...
      <select v-model.number="form.student_id" class="border rounded px-3 py-2">
        <option :value="null" disabled>Estudiante</option>
        <option v-for="s in students" :key="s.id" :value="s.id">{{ s.name }}</option>
      </select>
      <button type="submit" class="px-4 py-2 bg-slate-900 text-white rounded">Matricular</button>
    </form>
  </div>
</template>
